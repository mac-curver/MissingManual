...
static const double  bignum = 10000000000;                                      //MAXFLOAT/


- (char *) version {
	static char sccsversion[] = "ScientificImage 24.10.2003 by HJS from "
                                "pslib.c 1.30 22.11.2003";
	return sccsversion;
}
...
- (void) setFrame:(NSRect)frameRect {
	...
	trackingTag = [self addTrackingRect:frameRect owner:self userData:nil
                           assumeInside:NO
                   ];
        ...
        switch ([theEvent type]) {
					...
					switch (currentState) {
						case handClosedState:
							...
							if ((fabs(dx)>EPS) && (fabs(dy)>EPS)) {
								[self zoomInToRect:NSOffsetRect(
                                                [self bounds], dx, dy)
                                 ];
								origPoint = mouseLoc;
							}
            ...
            case NSEventTypeLeftMouseUp:
				switch (currentState) {
					...
					case zoomBottomLeftState:
					case zoomBottomRightState:
						if (dragged && (rubberbandRect.size.width>5)
                                    && (rubberbandRect.size.height>5))
                        {
...
- (IBAction) zoomIn:(id)sender {
	[self zoomInFromPoint:NSMakePoint(
                                viewRect.origin.x + viewRect.size.width/2,
                                viewRect.origin.y + viewRect.size.height/2)
     ];
}

- (IBAction) zoomOut:(id)sender {
	[self zoomOutFromPoint:NSMakePoint(
                                viewRect.origin.x + viewRect.size.width/2,
                                viewRect.origin.y + viewRect.size.height/2)
     ];
}
...
- (void) zoomInFromPoint:(NSPoint)mouseLoc {
	...
	[self windowFromX:myLocation.x+xLeft ToX:myLocation.x+xRight
             AndFromY:myLocation.y+yBottom ToY:myLocation.y+yTop
     ];
...
- (void) windowFromX:(double)xmin ToX:(double)xmax
...
/*
	...
	input:
		xmin, xmax, ymin, ymax: #define		MYFRAME		bounds
bounds to define the scales of the
...
- (void) drawString:(NSString *)markString alignment:(int)textAlignment {
	NSSize				size;
...
- (void) drawTriangleSymbol:(int)symbolDirection {
	NSPoint			A; 
	NSPoint			B;
	NSPoint			C;
...
- (void) drawCenteredSymbol:(int)symbolCode {
	...
	switch (symbolCode) {
        ...
        case 3:
            ...
            break;
        case 1: {
                NSPoint topLeft     = NSMakePoint(cursorPosition.x-symbolSize/2
                                                , cursorPosition.y-symbolSize/2
                                      );
                NSPoint bottomLeft  = NSMakePoint(cursorPosition.x-symbolSize/2
                                                , cursorPosition.y+symbolSize/2
                                      );
                NSPoint topRight    = NSMakePoint(cursorPosition.x+symbolSize/2
                                                , cursorPosition.y-symbolSize/2
                                      );
                NSPoint bottomRight = NSMakePoint(cursorPosition.x+symbolSize/2
                                                , cursorPosition.y+symbolSize/2
                                      );
			...
			break;
        case 2: {
                NSPoint top         = NSMakePoint(cursorPosition.x
                                                  , cursorPosition.y-symbolSize/2
                                                  );
                NSPoint bottom      = NSMakePoint(cursorPosition.x
                                                  , cursorPosition.y+symbolSize/2
                                                  );
                NSPoint right       = NSMakePoint(cursorPosition.x+symbolSize/2
                                                  , cursorPosition.y
                                                  );
                NSPoint left        = NSMakePoint(cursorPosition.x-symbolSize/2
                                                  , cursorPosition.y
                                                  );
            ...
            break;
        case 4:
            [NSBezierPath strokeRect:enclosingRect];
            break;
        default:
