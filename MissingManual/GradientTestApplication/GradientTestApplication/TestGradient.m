...
//  changed by LefoEsprit on 29.05.21.
...
- (void)addCircle:(NSPoint)point :(int)layerIndex {
    ...
    if (self.layer.sublayers[layerIndex]) {
        [self.layer replaceSublayer:self.layer.sublayers[layerIndex]
                               with:shapeLayer];
    }
    else {
...
- (IBAction)myPanGesture:(NSPanGestureRecognizer *)sender {
    ...
    NSPoint mouseLoc = [sender locationInView:self];
    NSLog(@"%@ - %@", NSStringFromPoint(mouseLoc), NSStringFromPoint([sender locationInView:self]));
